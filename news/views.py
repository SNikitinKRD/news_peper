...
class NewsList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.debug('получения контекста списка новостей')
        logger.info('получения контекста списка новостей')
        logger.warning('получения контекста списка новостей')
        logger.error('получения контекста списка новостей')
        return context
...
class NewsDetail(PermissionRequiredMixin, DetailView):
    ...
    def get_object(self, *args, **kwargs):  # переопределяем метод получения объекта
        instance = cache.get(f"post-{self.kwargs.get('pk')}", None)
        if not instance:
            instance = super().get_object(queryset=self.queryset)
            cache.set(f"post-{self.kwargs.get('pk')}", instance)
        return instance
